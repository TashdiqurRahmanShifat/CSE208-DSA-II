...
void kruskal(int n,float k,int parent[],int size[],vector<pair<float,pair<int,int>>>&weighted_graph,vector<pair<int,int>>&res)
{
    for(int i=0;i<n;i++)
        make(i,parent,size);
    float totalcost=0;
...
int main()
{
    ...
    kruskal(n,k,parent,size,weighted_graph,res);
